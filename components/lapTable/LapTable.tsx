...
const LapTable: FC<Props> = (props: Props) => {
  ...
  const [data, setData] = useState<UsedExerciseArrayItem[]>(
    props.exercises ?? []
  );
  ...
  const handleSelect = (e: SyntheticEvent) => {
    setSelectedExercise({
      ...
      title:
        props.exerciseList.find(
          (exercise) => exercise.id === (e.target as HTMLInputElement).value
  ...
  const handleDeleteButton = (e: SyntheticEvent, id: string) => {
    e.preventDefault();
  ...
  return (
    <>
      <label className="input-label" htmlFor="title">
        Exercises in lap #{props.lapIndex + 1}
      </label>
      <div className="block w-full">
        <table className="w-full bg-gray-50 text-gray-700 border border-slate-300 py-3 mb-3 leading-tight">
          ...
          <tbody>
            {data &&
              data.map((exercise: UsedExerciseArrayItem, i: number) => (
                <LapRow
                  id={exercise.usedExercise.id}
                  title={exercise.usedExercise.exercise.title}
                  reps={exercise.usedExercise.reps ?? 0}
                  timed={exercise.usedExercise.timed ?? false}
            ...
            <tr>
              <td className="py-4 pl-4" colSpan={3}>
                ...
                <button
                  ...
                  disabled={showSelect && selectedExercise === null}
                >
                  Add{!showSelect && " exercise"}
                </button>
              </td>
