...
const LapRow: FC<Props> = (props: Props) => {
  ...
  const updateReps = (e: SyntheticEvent, direction: string) => {
    ...
    if (direction === "minus" && reps > 0) setReps(reps - 1);
  };
  ...
  return (
    <tr className="w-full border-b">
        ...
        <div className="xt-list flex-nowrap justify-center" data-xt-groupnumber>
          <div className="inline-flex">
            ...
            <input
              type="number"
              value={reps}
              className="outline-none focus:outline-none text-center w-full bg-gray-300 font-semibold text-md hover:text-black focus:text-black md:text-basecursor-default flex items-center text-gray-700 appearance-none"
              name="custom-input-number"
              onInput={handleInput}
              min="0"
            />
            <button
              onClick={(e) => updateReps(e, "plus")}
      ...
      <td className="text-center">
        <button
          className="border px-1 rounded-full text-red-600 bg-white border-red-800 text-lg leading-5 align-text-top"
